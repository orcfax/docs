...
feeds, integrators (e.g. dApps, smart contracts) must use $FACT for payment.
While integrators may also use ADA, these payments are exchanged for FACT
tokens, creating a positive feedback loop that boosts demand for the FACT token.

Since integrators pay for on-chain publications, an increase in integrators
leads to more publications, which thereby drives up the demand for the FACT
token. The generated $FACT payments are distributed to entities crucial for
network operation and security: Orcfax Validators and the Orcfax network.

![Orcfax Economic Model](/img/2024-02--Orcfax-economic-model2.jpg)

Initially, with a relatively low number of integrators and publications after
the network launches on mainnet, validators and the Orcfax network will receive
compensation through the '[Validator Rewards Allocation][rewards-1]'. This
allocation, constituting 50% of the total FACT token supply, rewards Validators
and the Orcfax Network. The amount of FACT tokens per publication from this
allocation will decrease over time. As more integrators start using Orcfax
oracle feeds, the increasing FACT payments from these customers will compensate
for the reduced emission from the Validator Rewards Allocation, and will
eventually replace them entirely.
