...
This document aims to explain how dapp developers (ie integrators) can integrate Orcfax feeds into their dapp
...
A statement is included in the datum of a transaction input, and thus available to dapps via the reference input field.
...
The presence of an FS token in a UTXO verifies that the associated statement is authentic.
...
without having to roll their own update mechanism.
...
> Note: only the statement part of the datum is relevant to integrators.
